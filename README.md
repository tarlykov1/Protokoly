...
Проверка интерфейса: Node.js 24, `npm ci --ignore-scripts`, `npm test`. Node/jsdom нужны только для разработки и CI. В рабочем окружении обязательно настроить авторизацию через доверенный прокси; параметры и заголовки приведены в отчёте аудита.
...
Приложение содержит REST-адаптер Битрикс24, DOCX-парсеры, редактор и проверки ролей. Ограничения публикации после сбоев, прав на отдельные проекты и рабочего контура перечислены в отчёте аудита. Исторические разделы ниже описывают развитие MVP и не являются подтверждением готовности к продуктиву.
